...
my $LOG;
my $logfile="/tmp/logout.log";
open $LOG, '>>', $logfile or die "Cannot open $logfile: $!";

print { $LOG } $q->header('text/plain') or die "Cannot write to $logfile: $!";
print { $LOG } "Got the following headers:\n" or die "Cannot write to $logfile: $!";
for my $header ( keys %headers ) {
    print { $LOG } "$header: $headers{$header}\n" or die "Cannot write to $logfile: $!";
}
print { $LOG } "LOGOUT_FILE: " . EBox::CaptivePortal->LOGOUT_FILE . "Got the following headers:\n" or die "Cannot write to $logfile: $!";
close $LOG or die "Cannot close $logfile: $!";
system('cat ' . EBox::CaptivePortal->LOGOUT_FILE . '>/tmp/lofile.txt');
