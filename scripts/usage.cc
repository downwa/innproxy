...
void parselog1(int lLines) {
	char buf[65536];
	char buf2[65536];
	FILE *fp=popen("/home/administrator/scripts/tailogcp","r");
	if(!fp) { syslog(LOG_ERR,"/home/administrator/scripts/tailogcp: %s",strerror(errno)); exit(1); }
	int count=0;
...
int logLines() {
	FILE *fp=popen("/home/administrator/scripts/loglines","r");
	if(!fp) { syslog(LOG_ERR,"/home/administrator/scripts/loglines: %s",strerror(errno)); exit(1); }
	char buf[10]={0};
